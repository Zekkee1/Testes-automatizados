...
public class LoginStep {
    ...
    public void inserir_os_dados_do_usuario(DataTable dataTable) {
        ...
        loginFunc.accLogin(email,password);
    }
    @Then("devo visualizar o user {string} logado")
    public void devo_visualirar_o_user_logado(String usuario) {
        Assert.assertEquals(usuario,loginFunc.userEquals());
    }
    @Then("devo visualizar a mensagem de erro {string}")
    public void deve_visualizar_a_mensagem_de_erro(String erro) {
        Assert.assertEquals(erro,loginFunc.erromess());
    }
}
